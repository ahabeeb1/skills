...
**Refreshed by:** full-repo audit pass (hand reconciliation per the template's "edit by hand whenever it's wrong" rule; not a Phase 0 run — the next `prior-art-research` Phase 0 remains the canonical automated writer per ADR-0005). Counts reconciled to disk: 19 skills, 29 ADRs (24 integer + 5 dated), v1.28.0 current, 6 hooks, 23 glossary concepts, 4 dispatch records, 5 chain-shared references. The v1.28.0 audit follow-up removed the unwired cross-session halt-UX (actions/halt-ux/trust scripts + their tests) and unified the two grill extensions onto a shared `references/grill-extension-protocol.md`.
...
- **Chain depth:** Human layer (research → Design → grill + sign-off) then Machine layer (vertical-slice → tdd → release); `decision-record` and `write-plan` are conditional (one-way-door decisions / multi-phase work only); `verify-output` gates each tdd-loop commit (not a separate link). `agent-factors-check` and `devex-review` are conditional extensions of grill. `security-audit` is a standalone slash-invokable skill. 5 primitives (parallel-dev, deep-modules, vertical-slice, using-worktrees, systematic-debugging) + 2 meta (using-habeebs-skill, setup-habeebs-skill) + cross-session-detect. Every chain run executes at a depth tier — Quick / Balanced / Deep (ADR-0016).
- **ADR count:** 29 — two-era naming scheme LIVE: 24 frozen integer ADRs (0001-0024) + 5 dated ADRs (`2026-05-28-decouple-decision-identity-from-releases.md` shipped v1.23.0; `2026-06-01-chain-fidelity-executable-assertions.md` shipped v1.24.0; `2026-06-09-add-regrill-edge-and-grill-alignment-axes.md` shipped v1.25.0; `2026-06-10-loop-harness-fresh-context-outer-loop.md` shipped v1.26.0; `2026-06-13-remove-unwired-halt-ux.md` shipped v1.28.0). New ADRs are dated `YYYY-MM-DD-<slug>.md` at creation; old integers frozen, never renamed; new↔old cross-refs by title+markdown-link. Recent batch: ADR-0022 (behavioral-only SKILL.md body, shipped v1.21.0), ADR-0023 (v1.22.0 methodology bundle), ADR-0024 (plugin supply-chain threat model — deferred hardening), 2026-05-28-decouple-decision-identity (dated artifact naming; FULLY supersedes ADR-0020's late-binding-ID half, RE-STATES its Changesets half as in-force, shipped v1.23.0), 2026-06-01-chain-fidelity-executable-assertions (SP2 archaeology lint + SP3 fixture-ID rule + SP4 supersession check, shipped v1.24.0) + ADR-0003 Rule 4 in-place amendment (resolve-from-target hook predicate, v1.24.0).
- **Current release:** v1.28.0 (audit follow-up — removed unwired cross-session halt-UX (~400 lines: actions/halt-ux/trust scripts + 3 test suites), unified grill-extension skills onto shared `references/grill-extension-protocol.md`; ADR `2026-06-13-remove-unwired-halt-ux`). Preceded by v1.27.0 (full skill-base audit — hook I/O contract fixes to `hookSpecificOutput` format, chain fidelity, release flow alignment, portability) and v1.26.0 (loop harness — fresh-context-per-slice outer loop with failure-triage and tiered halt policy; ADR `2026-06-10-loop-harness-fresh-context-outer-loop`).
...
- **Latest ADR:** `2026-06-13-remove-unwired-halt-ux` (shipped v1.28.0). Recent dated ADRs: `2026-06-10-loop-harness-fresh-context-outer-loop` (v1.26.0); `2026-06-09-add-regrill-edge-and-grill-alignment-axes` (v1.25.0); `2026-06-01-chain-fidelity-executable-assertions` (v1.24.0); `2026-05-28-decouple-decision-identity-from-releases` (v1.23.0). Last integer ADR: ADR-0024 (plugin supply-chain threat model, deferred hardening). See `docs/agents/adrs/README.md` for the full index.
...
- **brownfield, methodology-mature** — habeebs-skill v1.28.0 is current on `main`. Recent release line: v1.24.0 (chain-fidelity executable assertions); v1.25.0 (Grill 2.0 — slice-shape axis + re-grill edge + mental-model probes); v1.26.0 (loop harness — fresh-context outer loop); v1.27.0 (full skill-base audit — hook I/O contract, chain fidelity, release flow); v1.28.0 (audit follow-up — halt-UX removal, grill-extension unification). v1.13.0 intentionally skipped (gstack spec staggered, shipped as v1.14.0). Self-dogfooded every cycle.
