...
- **ADR count:** 26 — two-era naming scheme LIVE: 24 frozen integer ADRs (0001-0024) + 2 dated ADRs (`2026-05-28-decouple-decision-identity-from-releases.md` shipped v1.23.0; `2026-06-01-chain-fidelity-executable-assertions.md` shipped v1.24.0). New ADRs are dated `YYYY-MM-DD-<slug>.md` at creation; old integers frozen, never renamed; new↔old cross-refs by title+markdown-link. Recent batch: ADR-0022 (behavioral-only SKILL.md body, shipped v1.21.0), ADR-0023 (v1.22.0 methodology bundle), ADR-0024 (plugin supply-chain threat model — deferred hardening), 2026-05-28-decouple-decision-identity (dated artifact naming; FULLY supersedes ADR-0020's late-binding-ID half, RE-STATES its Changesets half as in-force, shipped v1.23.0), 2026-06-01-chain-fidelity-executable-assertions (SP2 archaeology lint + SP3 fixture-ID rule + SP4 supersession check, shipped v1.24.0) + ADR-0003 Rule 4 in-place amendment (resolve-from-target hook predicate, v1.24.0).
- **Current release:** v1.24.0 (chain-fidelity hardening — scenario 34 self-referential-archaeology lint + 35 fixture-ID late-binding rule + 36 supersession-link integrity check; ADR-0003 Rule 4 amendment locks the v1.23.0 resolve-from-target hook fix as doctrine; runtime-lag docs; HABEEBS_DISABLE_HOOKS dev stopgap + HOOK-FIX-TODO.md removed; Changesets machinery UNTOUCHED).
- **Dogfood scenario count:** 39 live (5 baselines + 21b/22-36 + adversarial suites like 09-category-critic, 10-pgroup-dispatch, 17-security-audit fixtures). Known reds on main: 19 (cross-session-conflict-detection) + 29 (chain-state-validator) — pre-existing, NOT regressions, untriaged. (Prior SYSTEM_CONTEXT said "16+29"; corrected v1.24.0 — 16 now passes, 19 is the actual red; surfaced by the release editorial scan.)
