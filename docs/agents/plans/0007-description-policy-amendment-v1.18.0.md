# Plan: Apply the v1.18.0 description-policy amendment — anatomy rewrite + auto-invocation scope + routing primer

| Field         | Value                                                                                  |
|---------------|----------------------------------------------------------------------------------------|
| Plan ID       | `plans/0007-description-policy-amendment-v1.18.0`                                      |
...
**Acceptance gate:** ADR-0007 carries the 2026-05-24 amendment section in `main`; 6 OQs all DECIDED; grill record committed at `docs/agents/specs/v1.18.0-auto-trigger-reliability-grill.md`. **GATE PASSED** during the chain run that produced this plan.
...
2. v1.19.0 fallback variant assumption (`You MUST use this skill when…`) is also too advisory — mitigation: phase 5's release captures a pre-rewrite eval baseline so v1.19.0 has comparison data either way.
...
**Acceptance gate:** **`bash tests/dogfood/11-description-budget/check-description-budget.sh` exits 0 against the v1.18.0 branch AND exits 1 against `main` pre-merge (regression-test the negative). All 3 SKILL.md files in Slice 7 reference the amended ADR-0007 section A-F.**
...
**Acceptance gate:** **Tag `v1.18.0` exists on `origin`. `plugin.json` and `marketplace.json` reflect `1.18.0`. `CHANGELOG.md` v1.18.0 entry has a Why line citing trigger-firing reliability and the ADR-0007 amendment. PR merged to `main`.**

**Top risks:**
1. The doc-sync coverage audit in `release` skill (per Slice 7 amendment) catches an undocumented change. Mitigation: Slice 7 specifically updates the audit's coverage; this is the audit's *first* run on the v1.18.0 template, so any miss surfaces as a release-skill error, not a silent gap.
...
3. Post-release, the 30-day transcript eval (Slice 6 cadence) shows <10pp lift — gating the v1.19.0 candidate. Mitigation: this is *expected behavior* per the success metric. v1.19.0 is the planned response, not a regression.
...
| #4  | Update `check-description-budget.sh` for v1.18.0 policy | AFK:full-auto      | 4     | pgroup-4A  | #1, #3     | 0.5d | `git revert` dogfood script commit                     |
| #7  | Doc-sync `release`/`setup`/`using-habeebs-skill` | AFK:full-auto      | 4     | pgroup-4A  | #1, #2, #3 | 0.5d | `git revert` doc-sync commits                          |
| #8  | Release v1.18.0 (version bump, CHANGELOG, PR, tag) | HITL:approval-gate | 5     | pgroup-5A  | all        | 0.5d | v1.18.1 supersedes (no tag deletion)                   |
...
| R2  | 1     | v1.19.0 fallback assumption is also too advisory                                                  | Medium     | Low    | Phase 5 captures pre-rewrite baseline so v1.19.0 has comparison data regardless                          |
...
| R13 | 5     | Tag-push hook blocks v1.18.0 push                                                                 | Low        | High   | ADR-0015 allows tag-only pushes on default branch; verify hook honors that pre-push                     |
| R14 | 5     | +30-day transcript eval shows <10pp lift                                                          | Medium     | Low    | This is expected v1.19.0 trigger, not regression. v1.19.0 candidate already scoped (`You MUST use...`)  |
...
- Spec: [`specs/v1.18.0-auto-trigger-reliability`](../specs/v1.18.0-auto-trigger-reliability.md)
- Grill: [`specs/v1.18.0-auto-trigger-reliability-grill`](../specs/v1.18.0-auto-trigger-reliability-grill.md)
