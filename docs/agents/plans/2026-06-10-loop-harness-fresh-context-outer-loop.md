...
- 2026-06-10 — Phase 2 GATE PASSED. pgroup-2A (#3, #5) landed via disp-v1260-2a (merges 0774643/2492acb): scenarios 43 (7/7), 44 (9/9) green; full Phase-1 regression set green; reviewer PASS (9 criteria, 0 findings, 3 invariants). ADR-0004 amended in place; loop mode + tiered halt policy + /tdd --resume live in tdd-loop. Only #6 (docs sync + changeset) remains.
- 2026-06-10 — Slice #6 landed (sequential, in-tree): GLOSSARY +5 terms, chain-at-a-glance shows loop mode, Grill 2.0 ADR line-76 trigger discharged in its changelog, minor changeset written. Fix applied en route: slices 3/5 had introduced inline ADR-cite lint violations in two SKILL.md bodies (scenario 26 caught them); de-cited. Full dogfood sweep: 1 red total — pre-existing scenario 16. Phase 3 gate criterion (1) (doc-sync audit zero WARN) is verified by the release skill at release time; criteria (2) and (3) verified green. Plan remains Active until the v1.26.0 release ships.
