...
### Phase 3 — DROPPED (audit reversal mid-implementation; see adr-methodology-folder-cuts § 2026-05-25 Amendment)

Slices #4 and #5 dropped during their own prep. Runtime-writer inventory revealed both `dispatches/` (parallel-dev Phase 7.5 writer) and `conflicts/` (cross-session-detect/audit.sh writer) are runtime audit-log targets, not dormant methodology folders. Pattern G doesn't apply. Both directories preserved; ADR-0004 Part 2 + ADR-0018 Part A preserved (no tombstones); `adr-methodology-folder-cuts.md` amended in place to document the reversal. Phase 3 acceptance gate is therefore trivially satisfied (no work to do). Plan advances directly from Phase 2 → Phase 4.

<details><summary>Original Phase 3 description (superseded)</summary>
...
- 2026-05-25 — Status flipped to Active. Phase 1 gate SATISFIED: Slice #2 landed as commit `09d20b9` (`.changeset/` substrate + scenario 22); Slice #1 landed as commit `3bd2d08` (late-binding plumbing + scenario 21 5/5 cases pass). All 5 baseline dogfood tests still pass. Next: Phase 2 pgroup-2A = {Slice #3, Slice #6}.
- 2026-05-25 — Phase 2 gate SATISFIED: Slice #6 landed as commit `f309e0b` (grill-records → specs/<name>-grill.md fold + scenario 24a 3/3 pass); Slice #3 landed as commit `7721e58` (release-skill aggregation + path-audit phases + scenarios 23 5/5 + 25 5/5). All 10 dogfood scenarios pass.
- 2026-05-25 — Phase 3 DROPPED. Runtime-writer inventory during Slice 4 prep revealed `dispatches/` (parallel-dev Phase 7.5) and `conflicts/` (cross-session-detect/audit.sh) are runtime audit-log targets, not dormant methodology folders. `adr-methodology-folder-cuts.md` amended in place reversing both deletions; ADR-0004 Part 2 + ADR-0018 Part A preserved. Plan advances directly to Phase 4 (Slice #7 self-dogfood release).
