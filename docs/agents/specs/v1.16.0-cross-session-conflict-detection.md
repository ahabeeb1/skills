...
**Spec'd from:** [ADR-0018](../adrs/0018-amend-adr-0002-for-advisory-in-flight-reads.md) + [research](../research/2026-05-22-cross-session-conflict-detection.md) + [grill](../grill-records/2026-05-22-cross-session-conflict-detection.md)
...
| ADR-0002 carve-out | New ADR-0018 with four-sub-clause guard (a)-(d) | Pattern matches ADR-0005/6/10 expanding ADR-0001 |
...
- [x] ADR-0002 carve-out shape — locked four-sub-clause guard in ADR-0018
...
**Notes:** Menu vocabulary, keystroke set, and pager integration are fully specified in the spec and ADR-0018 — no mid-flow human input needed. End-of-slice pager-feel review handled by `tdd-loop` Phase 5 code-quality review, not by HITL gating.
...
**Description:** Update `docs/agents/SYSTEM_CONTEXT.md` (note ADR-0018, new directories under `docs/agents/conflicts/` + `.git/habeebs-sessions/`), add `CHANGELOG.md` entry for v1.16.0, amend or add skill README(s) for the new mechanism, update the ADR-0002 status field with the forward-reference to ADR-0018 + the ADR index README to list ADR-0018.

**Acceptance criteria:**
- [ ] `SYSTEM_CONTEXT.md` updated with v1.16.0 + ADR-0018 + new tracked directory `docs/agents/conflicts/` + new runtime directory `.git/habeebs-sessions/`
- [ ] `CHANGELOG.md` v1.16.0 entry references ADR-0018 + the dogfood suite outcomes
- [ ] ADR-0002 status → `Accepted (amended by 0018)`; References section adds forward link
- [ ] `docs/agents/adrs/README.md` index lists ADR-0018
...
**Notes:** The ADR-0002 status amendment + README index entry are intentionally deferred to this slice (rather than at ADR-0018 write-time) so they land together with the implementation that actually fulfills the carve-out — keeping the index consistent with shipped functionality.
...
- Collab usage scales beyond occasional same-checkout sessions → re-evaluate Alternative 4 (server-mediated locking) from ADR-0018
...
- A second mechanism wants the ADR-0018 carve-out (e.g., long-running spike traces) → pressure-test the four-sub-clause guard
