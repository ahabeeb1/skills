...
Idea-ported from [gstack](https://github.com/garrytan/gstack)'s `/ship` skill (MIT) — the version-bump + CHANGELOG + PR-body + tag-push release assembly pattern. The doc-sync sub-phase is ported from gstack's `/document-release` — coverage-map of shipped features lacking docs and the CHANGELOG "sell test" (entries explain value, not just changes). Per ADR-0014, the methodology is portable; this skill's text is written independently. Dropped from gstack `/ship`: deploy pipeline execution, canary promotion, benchmark gating, and cross-run learnings ledger (`/learn`) — all require runtime substrate or production access that habeebs-skill forbids (ADR-0002).
