...
3. **verify-output variant.** Same shape on the Pass 4c surface: a verify-output `BLOCKED` gets exactly 1 fresh-context fix attempt; the same finding surviving the fix → halt, never a second identical attempt.
